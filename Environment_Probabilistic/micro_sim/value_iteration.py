"""Bellman value iteration on the intersection MDP (stochastic translation).

For translation actions, the Bellman backup uses the **expectation** over
stochastic next states:

    Q(s, a) = sum_{s'} P(s' | s, a) [ R(s, a, s') + gamma * V(s') ]

Rotations (oriented ``L`` / ``R``) remain deterministic (single ``s'``).
"""

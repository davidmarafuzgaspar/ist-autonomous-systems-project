...
def _print_formula(gamma: float, theta: float) -> None:
    print(
        "Bellman optimality (stochastic translation, expectation over s'):\n"
...
def main() -> None:
    ...
    parser.add_argument("--seed", type=int, default=42, help="RNG seed for stochastic rollouts")
    parser.add_argument(
        "--p-forward",
        type=float,
        default=None,
        help="translation: intended direction weight (default 0.6; renormalized with p-left/p-right)",
    )
    parser.add_argument("--p-left", type=float, default=None, help="translation: slip perpendicular left weight")
    parser.add_argument("--p-right", type=float, default=None, help="translation: slip perpendicular right weight")
    ...
    rollout_rng = random.Random(args.seed)
    ...
    if args.interactive:
        ...
        print(
            ...
            f"Translation slip weights (renormalized): forward={world.motion_prob_forward}, "
            f"left={world.motion_prob_left}, right={world.motion_prob_right} "
            f"-> P={tuple(round(x, 4) for x in world.normalized_motion_probs())}\n"
        )
    ...
    print(
        ...
        + (
            ...
            f"turn_90={world.turn_90_reward}\n"
            f"Translation slip weights -> P={tuple(round(x, 4) for x in world.normalized_motion_probs())}\n"
            if world.oriented_mdp
            else f"Cell MDP translation slip -> P={tuple(round(x, 4) for x in world.normalized_motion_probs())}\n"
        )
    )
    ...
    _roll_out_policy(world, result.policy, args.rollout_steps, rollout_rng)
